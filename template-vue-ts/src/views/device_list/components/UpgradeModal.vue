    ...
    <n-form class="w-100%" label-placement="left">
      ...
      <n-form-item label="版本列表">
        <div class="flex w-100% gap-16px">
          ...
          />
          <n-button type="primary" :loading="loading" @click="handleUpgrade"
            >升级</n-button
          >
        </div>
      </n-form-item>
...
const title = computed(() => (props.type === "1" ? "升级4G固件" : "升级MCU"));
...
const packageOptions = computed(
  () =>
    (props.type === "1" ? store.packageListWith4G : store.packageListWithMCU) ||
    [],
);

onMounted(() => {
  if (props.type && packageOptions.value.length <= 0) {
    store.getPackageList(props.type);
  }
});
...
// 进度条动画
const animationProgress = (target, duration) => {
