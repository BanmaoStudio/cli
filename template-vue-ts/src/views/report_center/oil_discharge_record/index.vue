...
import { useOilDischargeStore } from './store'
import { useList } from '@/hooks'
import { OilDischargeRequest, OilDischargeItem } from './types'
import { fetchOutRecordExportList } from './service'
import {
    areaOptions,
    areaValue,
    loadAllAreaData,
...
const pageTitle = computed(() => {
    return (route.meta.title as string) || '出油记录'
})
const rowKey = (row: RowData) => row.id
const show = ref(false)
type SearchParams = Omit<OilDischargeRequest, 'pageNum' | 'pageSize'>
...
const store = useOilDischargeStore()
/** 获取列表数据 */
const {
    dataSource,
    loading,
